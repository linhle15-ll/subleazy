...
// --- Lib ---
import { isNodeInSchema } from "@/lib/utils/tiptap-utils"
...
export function HeadingDropdownMenu({
  ...
  const getActiveIcon = React.useCallback(() => {
    if (!editor) return <Heading className="tiptap-button-icon" />

    const activeLevel = levels.find((level) =>
      editor.isActive("heading", { level })
    ) as Level | undefined

    if (!activeLevel) return <Heading className="tiptap-button-icon" />
