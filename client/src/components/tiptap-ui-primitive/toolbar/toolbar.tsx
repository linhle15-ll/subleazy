...
import "@/components/tiptap-ui-primitive/toolbar/toolbar.module.css"
...
const useToolbarKeyboardNav = (
  ...
  React.useEffect(() => {
    ...
    const focusableElements = getFocusableElements()
    Array.isArray(focusableElements) && focusableElements.forEach((element) => {
...
export const Toolbar = React.forwardRef<HTMLDivElement, ToolbarProps>(
  ({ children, className = "", variant = "fixed", ...props }, ref) => {
    ...
    // Tailwind styles for the toolbar
    const baseToolbar =
      "flex items-center gap-1 w-full min-h-[2.75rem] bg-white px-2 overflow-x-auto overscroll-contain sticky top-0 z-10"
    const floatingToolbar =
      "p-1.5 rounded-lg border border-gray-200 bg-white shadow-lg outline-none overflow-hidden"
    const mobileToolbar =
      "fixed bottom-0 left-0 right-0 h-[calc(2.75rem+env(safe-area-inset-bottom,0px))] border-t border-gray-200 border-b-0 px-2 pb-[env(safe-area-inset-bottom,0px)] flex-nowrap justify-start"

    let toolbarClass = baseToolbar
    if (variant === "floating") {
      toolbarClass = floatingToolbar
    }
    if (variant === "fixed" && typeof window !== "undefined" && window.innerWidth <= 480) {
      toolbarClass = mobileToolbar
    }
    ...
    return (
      <div
        ...
        className={`${toolbarClass} ${className}`}
        {...props}
      >
        {children}
      </div>
...
export const ToolbarGroup = React.forwardRef<HTMLDivElement, BaseProps>(
  ({ children, className = "", ...props }, ref) => {
    ...
    // Tailwind styles for the toolbar group
